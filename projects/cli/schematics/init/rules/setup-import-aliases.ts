  ...
  return (tree: Tree, context: SchematicContext) => {
    try {
      ...
      if (!tree.exists(tsConfigPath)) {
        context.logger.warn('tsconfig.json not found, skipping import alias setup');
        return tree;
      }
      ...
      if (tsConfigContent.includes('@workspace/ui/*')) {
        context.logger.info('Import alias @workspace/ui/* already exists, skipping');
        return tree;
      }
      ...
      if (tsConfigContent.includes('"paths"')) {
        ...
        if (match) {
          ...
          const newPathsContent = pathsContent.trim() + 
        ...
        if (match) {
          ...
          const newCompilerOptionsContent = compilerOptionsContent.trim() + 
            (compilerOptionsContent.trim().endsWith(',') ? '' : ',') + 
            '\n    "paths": {\n      "@workspace/ui/*": ["./ui/*"]\n    }';
          
          updatedContent = tsConfigContent.replace(
            compilerOptionsRegex,
            beforeCompilerOptions + newCompilerOptionsContent + afterCompilerOptions
          );
        }
      }
