...
const sheetVariants = tv({
    slots: {
        sheet: 'bg-background data-[enter]:animate-in data-[exit]:animate-out fixed z-50 flex flex-col gap-4 shadow-lg transition ease-in-out data-[enter]:duration-500',
        sheetOverlay: 'data-[enter]:animate-in data-[exit]:animate-out data-[exit]:fade-out-0 data-[enter]:fade-in-0 fixed inset-0 z-50 bg-black/50',
    ...
    variants: {
        side: {
            right: {
                sheet: 'data-[exit]:slide-out-to-right data-[enter]:slide-in-from-right inset-y-0 right-0 h-full w-3/4 border-l sm:max-w-sm'
            },
            left: {
                sheet: 'data-[exit]:slide-out-to-left data-[enter]:slide-in-from-left inset-y-0 left-0 h-full w-3/4 border-r sm:max-w-sm'
            },
            top: {
                sheet: 'data-[exit]:slide-out-to-top data-[enter]:slide-in-from-top inset-x-0 top-0 h-auto border-b'
            },
            bottom: {
                sheet: 'data-[exit]:slide-out-to-bottom data-[enter]:slide-in-from-bottom inset-x-0 bottom-0 h-auto border-t'
            }
        }
    },
    defaultVariants: {
...
const { sheet, sheetOverlay, sheetHeader, sheetFooter, sheetTitle, sheetDescription, sheetTrigger } = sheetVariants();
...
@Directive({
    ...
    hostDirectives: [
        { directive: NgpDialog,
            inputs: [
                'ngpDialogRole: uiSheetRole',
                'ngpDialogModal: uiSheetModal'
            ] 
        }
    ]
})
...
export class UiSheetDescription {
    inputClass = input<string>('', { alias: 'class' });
    computedClass = computed(() => sheetDescription({ class: this.inputClass() }));
}
...
@Directive({
    ...
    hostDirectives: [
        { 
            ...
            inputs: [
                'ngpDialogOverlayCloseOnClick: uiSheetOverlayCloseOnClick'
            ]
        }
    ]
})
